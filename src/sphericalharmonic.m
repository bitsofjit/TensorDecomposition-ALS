...
% Purpose
% =======
% Get the value of the spherical harmonic Y_L^M and its derivatives at xg
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% $Y_{L}^{M}(\theta, \phi)$ OR $Y_{L}^{M}(\mathbf{r})$
% $ Y_{L}^{M}(\theta, \phi) = Const*\exp(i m \phi)*P_{L}^{M}(\cos\theta)$
% 
% 
% *************************************************************************
% $P_{L}^{M}$ == associated Legendre polynomial; 
% Const       == Normalization constant 
%             == \sqrt{\frac{(2*L + 1)(L - m)!}{4 \pi (L + m)!}}
% *************************************************************************
% 
% 
% $M$ is integer with $-L <= M <= L$ ==> $(2*L + 1)$ values including $0$
% $L = 0, 1, \ldots, \infty$
